...
bool Sudoku::generateBoard(){
  ...
  for (int row = 0; row <  power_; row++){
	...
	if (tested.size() >= power_){
      ...
      } while ((count(rows.at(row).begin(), rows.at(row).end(), randNum) +
		count(columns.at(column).begin(), columns.at(column).end(), randNum) +
		count(boxes.at((row / size_) * size_ + (column / size_)).begin(),
		      boxes.at((row / size_) * size_ + (column / size_)).end(), randNum)) > 0);
